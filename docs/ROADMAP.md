...
- **Comprehensive docs** (PR #42): the `/docs` page grew from 5 sections to 11 — Getting started, an end-to-end Tutorial, Framework (project layout + routing + the typed-loader contract + SSR + the toolchain API), Database (schema-as-code + migrations + RLS helpers + managed Postgres), full **CLI** (`podkit` + `podkit cloud`) and **REST API** references (every `/v1` route with method/auth), Concepts, a full env-var **Configuration** table, **Self-hosting** runbook, and **Troubleshooting** (error codes). Fixed real inaccuracies in the offline `@podkit/docs` corpus (no `action` export; RLS is opt-in helpers, not automatic; SSR renders, not streams) and added `cloud`/`branches`/`env` topics. Framework: added erasable `PageProps<T>` + `LoaderData<typeof loader>` types so route components stay in sync with their loaders (now used by `podkit init`). Verified the page renders (desktop + mobile, no console errors).
...
- [x] Account lifecycle: password reset, delete account, basic email verification — DONE (PR #51): session expiry+revocation (`sessions.expiresAt` now enforced + table-backed revoke), single-use hashed-at-rest reset tokens (anti-enumeration, rate-limited, revokes sessions), email-verify flag (not login-gated), `DELETE /v1/account` cascade. Dev email = a logged sink (`setEmailSender`-pluggable, no SMTP dep). Console UI wiring still a follow-up.
...
- [~] **Per-account quotas + API rate limiting** — DONE (PR #41): per-account **project quota** (`PODKIT_MAX_PROJECTS_PER_ACCOUNT`) + **API rate limiting** (`PODKIT_RATE_LIMIT_PER_MIN`, per credential/IP). Remaining: ✅ per-account **memory + container-count caps** DONE (PR #52: `PODKIT_MAX_MEMORY_MB_PER_ACCOUNT`/`PODKIT_MAX_CONTAINERS_PER_ACCOUNT`, enforced at deploy with a structured 429, machine key exempt, self-supersede not gated); still open: disk caps (not meterable locally) + an abuse/teardown policy.
...
2. **docker.sock host-escape** — the control-plane mounts the host Docker socket; move to a brokered build/run service or orchestrator. *(Architectural — needs sign-off.)* Smaller remaining container hardening: ✅ `--read-only` rootfs (+ capped noexec/nosuid/nodev `/tmp` tmpfs) and ✅ digest-pinnable base image (`PODKIT_BASE_IMAGE` accepts `name@sha256:…`) DONE (PR #52); still open: non-root control-plane (needs docker-group gid mapping), auto-resolving `:latest` → digest at build.
...
4. **App source delivery** — DONE for Dockerfile apps via **upload-based deploy** (prod #34 + preview #36, routing fixed in #35; full loop verified live). Remaining: (a) standalone **buildpack** apps still need published `@podkit/*` packages (publish them or bundle the framework into the generated image); (b) optional git-push / registry ingestion.
5. **Cold-start / edge** — prod bundling shipped (#28); ✅ **scale-to-zero + cold-start** DONE (PR #54, opt-in via `PODKIT_IDLE_TIMEOUT_MIN`): an idle-reaper stops a project's container after N minutes idle (using per-route `lastSeen`), and the gateway resolves a reaped project as **sleeping** — kicking off a wake (re-run the active deployment's cached image + `waitForReadiness`) while serving an auto-refreshing 503 holding page; startup also marks deployed projects sleeping so prod recovers lazily after a control-plane restart. Remaining: faster container cold-start (snapshot/checkpoint), edge runtime, SSE-pushed readiness instead of meta-refresh.
...
- **Auth:** OAuth (Google/GitHub), passkeys, magic links, SSO/SAML. (✅ **session token expiry + table-backed revocation** DONE (PR #51) — `sessions.expiresAt` enforced on resolve (NULL = non-expiring, backward-tolerant) + `revokeSession`/`revokeAllSessions`.)
- **DB:** branching (preview-deploy isolated branches). (✅ **Realtime subscriptions** — `@podkit/db` `createRealtime()` → `subscribe`/`notify` over Postgres LISTEN/NOTIFY (dedicated connection per sub, channel-name injection guard, dual-driver node-postgres/pglite, `notifyTriggerSql()` helper); LISTEN/NOTIFY only, no logical-replication replay. ✅ **Auto-generated REST** — `createRestHandler(client, table)` introspects a drizzle table → node:http CRUD (list+pagination/one/insert/update/delete) routed through the principal's client so RLS holds; column-whitelisted, PK-protected, parameterized, 1 MiB cap; CRUD only, no filter DSL/relations/OpenAPI.) (✅ `db pull` now **regenerates the TS schema** — introspection emits a drizzle `schema.ts` importing from `@podkit/db` alongside the SQL migration, so adopting an existing DB round-trips to code. ✅ `@podkit/db` now re-exports the drizzle query helpers — `eq`/`and`/`desc`/… — so apps depend only on `@podkit/*`; importing `drizzle-orm` directly broke standalone deploys.)
- **Framework:** ~~nested `_layout`~~ ✅ **DONE** — `app/routes/_layout.tsx` (root) and `<dir>/_layout.tsx` (nested) wrap a route's pages outermost-first; presentational (`{ children, data }`), SSR-compiled like routes, applied in both dev + prod servers. ~~Client hydration (the #1 framework correctness item)~~ ✅ **DONE** — the framework now owns the client entry: the server embeds the matched route id (`window.__PODKIT_ROUTE__`) alongside the data, and a generated route table + hydration bootstrap loads the same route+layout components, rebuilds the tree, and `hydrateRoot`s it — so `useState`/effects/handlers work in the browser. A Vite plugin (`podkitPlugins`) serves the virtual route table and **strips `loader`/`action` (+ their `node:`/`@podkit/db` imports) from the client bundle** so server-only code never ships. Verified end-to-end in jsdom against the real built bundle (SSR markup → click increments a counter). Apps no longer scaffold `entry-client.tsx`. ~~Still open: streaming SSR, static/ISR modes; route-aware client navigation (SPA links); per-`_layout` loaders.~~ ✅ **ALL DONE** (PRs #44–#47): **per-`_layout` loaders** (each layout runs its own `loader`, gets its own `data`, embedded as `__PODKIT_LAYOUT_DATA__` for hydration); **streaming SSR** (`renderToPipeableStream` in dev+prod via `renderPageToStream`); **static prerender + ISR** (`export const prerender`/`revalidate` → param-less routes prerendered at build, prod-server serves them + in-process stale-while-revalidate); **route-aware SPA navigation** (`<Link>` + client router fetching loader data as JSON via `x-podkit-data:1`, `popstate` for back/forward). ✅ SPA polish done too: `<Link>` **prefetches** loader data on hover/focus (one-shot cache) and **scroll is restored** on back/forward (saved per history entry, reset to top on forward nav). ✅ Render tail DONE (PR #50): **Suspense-aware streaming** (data/script tail emitted after all boundaries flush, so hydration always has its data) and **`getStaticPaths`** (a dynamic `prerender` route enumerates concrete param combos → one static HTML each at build, SSR fallback for the rest). Remaining tail (architectural): distributed ISR cache, `fallback:blocking`-style incremental static generation.
- **Telemetry:** alerting. (✅ **traces/spans** DONE (PR #53) — `createTracer`/`startSpan`/`withSpan` write `kind:"span"` events (traceId/spanId/parentId via `AsyncLocalStorage`) to the JSONL sink; OTLP export is the upgrade. ✅ **log `--follow`/`-f`** DONE (PR #53) — polling tail for local + cloud logs with tested cursor/dedup; SSE/websocket is the upgrade.)
