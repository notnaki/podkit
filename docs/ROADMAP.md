...
4. **App source delivery** — DONE for Dockerfile apps via **upload-based deploy** (#34, routing fixed in #35; verified live). Remaining: (a) **preview/`deploy-branch` still uses the old local-`contextDir` model** — needs an upload variant to work on a hosted cloud (prod `deploy` already does); (b) standalone **buildpack** apps still need published `@podkit/*` packages (publish them or bundle the framework into the generated image); (c) optional git-push / registry ingestion.
