...
- **Framework:** ~~nested `_layout`~~ ✅ **DONE** — `app/routes/_layout.tsx` (root) and `<dir>/_layout.tsx` (nested) wrap a route's pages outermost-first; presentational (`{ children, data }`), SSR-compiled like routes, applied in both dev + prod servers. Still: streaming SSR, static/ISR modes; production bundling; route-aware client navigation. **⚠ Client hydration is currently a no-op shell** — the scaffolded `app/entry-client.tsx` calls `hydrateRoot(root, root.firstElementChild as never)`, passing a DOM node where React expects an element, so there is no real component hydration (no client interactivity, no client data wiring). The honest current model is prop-based SSR (`{ data }` to the server-rendered component). Fixing this is the **#1 framework correctness item**: generate a client route table, match `location` → route module, and `hydrateRoot(root, <Component data={window.__PODKIT_DATA__} />)`. Until then, a `useLoaderData()` hook would be misleading, so we ship typed props (`PageProps`/`LoaderData`) instead.
