...
- **DB:** branching (preview-deploy isolated branches), realtime subscriptions, auto-generated REST, `db pull` regenerating the TS schema. (✅ `@podkit/db` now re-exports the drizzle query helpers — `eq`/`and`/`desc`/… — so apps depend only on `@podkit/*`; importing `drizzle-orm` directly broke standalone deploys.)
