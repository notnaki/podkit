...
4. **App source delivery** — DONE for Dockerfile apps via **upload-based deploy** (#34): the CLI tars + streams the app up, the control-plane extracts (traversal/symlink-safe) + builds. Remaining: standalone **buildpack** apps still need published `@podkit/*` packages (publish them, or bundle the framework into the generated image); optionally git-push / registry ingestion.
